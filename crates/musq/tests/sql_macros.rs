...
mod tests {
    ...
    // These helpers only prove that the macros expand in functions that do not
    // return `Result`; they execute nothing. Runtime behavior is covered by the
    // tests above.
    ...
    fn create_sql_as_simple() -> Map<impl FnMut(Row) -> Result<(i32, String)>> {
        sql_as!("SELECT 1 as id, 'hello' as name").unwrap()
    }

    fn create_sql_as_with_params() -> Map<impl FnMut(Row) -> Result<(i32, String)>> {
        let id = 42;
        let name = "world";
    ...
    fn create_sql_as_with_struct() -> Map<impl FnMut(Row) -> Result<TestRecord>> {
        sql_as!("SELECT 999 as id, 'struct test' as message").unwrap()
    }

    // Additional test showing sql_as! parameters in non-result context
    fn create_sql_as_with_complex_params() -> Map<impl FnMut(Row) -> Result<(i32, String, bool)>> {
        let user_id = 123;
        let status = "active";
        let is_admin = true;
        sql_as!(
