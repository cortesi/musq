use musq::{query, Pool, PoolConnection, Musq};
...
mod pool_execution {
    ...
    async fn test_execute_with_pool() -> musq::Result<()> {
        ...
        let result = pool.execute(query("INSERT INTO test_table (value) VALUES (?)")
            .bind("pool_value"))
            .await?;
    ...
    async fn test_fetch_one_with_pool() -> musq::Result<()> {
        ...
        let row: Row = pool.fetch_one(query("SELECT value FROM test_table WHERE id = ?")
            .bind(1))
            .await?;
    ...
    async fn test_fetch_optional_with_pool() -> musq::Result<()> {
        ...
        let row: Option<Row> = pool.fetch_optional(query("SELECT value FROM test_table WHERE id = ?")
            .bind(999))
            .await?;
    ...
    async fn test_fetch_all_with_pool() -> musq::Result<()> {
        ...
        let rows: Vec<Row> = pool.fetch_all(query("SELECT value FROM test_table ORDER BY id"))
            .await?;
    ...
    async fn test_concurrent_pool_execution() -> musq::Result<()> {
        ...
        let futures = (0..5).map(|i| {
            ...
            async move {
                pool.execute(query("INSERT INTO test_table (value) VALUES (?)")
                    .bind(&value))
                    .await
            }
        });
        ...
        let count: Row = pool.fetch_one(query("SELECT COUNT(*) as count FROM test_table"))
            .await?;
...
mod error_handling {
    ...
    async fn test_execute_invalid_sql_with_connection() -> musq::Result<()> {
        ...
        let result = query("INVALID SQL STATEMENT")
            .execute(&mut conn)
            .await;
    ...
    async fn test_execute_invalid_sql_with_pool() -> musq::Result<()> {
        ...
        let result = pool.execute(query("INVALID SQL STATEMENT"))
            .await;
