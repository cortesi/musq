use musq::{Executor, Connection, Musq, query, query_as};
use std::sync::Arc;
...
async fn test_executor_interchangeability() -> anyhow::Result<()> {
    ...
    // Verify pool data
    let pool_results: Vec<(i32, String)> = query_as("SELECT id, value FROM test_interop ORDER BY id")
        .fetch_all(&pool)
        .await?;
    ...
    let standalone_result: (i32, String) = query_as("SELECT id, value FROM test_standalone WHERE id = ?")
        .bind(3)
...
async fn test_pool_in_generic_function() -> anyhow::Result<()> {
    async fn insert_and_count<E: for<'a> Executor<'a> + Send + Sync>(executor: &E, table: &str, value: &str) -> anyhow::Result<i64> {
