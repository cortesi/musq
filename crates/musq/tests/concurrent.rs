...
use musq::{Connection, Musq, query, query_as, Executor};
...
async fn test_concurrent_reads() -> anyhow::Result<()> {
    ...
    // Setup test data
    conn.execute(query("CREATE TABLE test_concurrent_reads (id INTEGER, value TEXT)")).await?;
    for i in 0..10 {
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            let rows: Vec<(i32, String)> = query_as("SELECT id, value FROM test_concurrent_reads WHERE id >= ?")
                .bind(i)
...
async fn test_concurrent_executes() -> anyhow::Result<()> {
    ...
    // Setup test table
    conn.execute(query("CREATE TABLE test_concurrent_executes (id INTEGER, thread_id INTEGER)")).await?;
    ...
    for thread_id in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            for i in 0..5 {
                let result = conn_clone.execute(query("INSERT INTO test_concurrent_executes (id, thread_id) VALUES (?, ?)")
                    .bind(thread_id * 5 + i)
                    .bind(thread_id)).await;
                assert!(result.is_ok());
            }
        });
        handles.push(handle);
...
async fn test_concurrent_prepared_statements() -> anyhow::Result<()> {
    ...
    // Setup test table
    conn.execute(query("CREATE TABLE test_concurrent_prepared (id INTEGER, data TEXT)")).await?;
    ...
    for _ in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            let mut results = vec![];
            for i in 0..10 {
                let row: (i32, String) = query_as("SELECT id, data FROM test_concurrent_prepared WHERE id = ?")
                    .bind(i * 10)
...
async fn test_concurrent_read_write_mix() -> anyhow::Result<()> {
    ...
    // Setup test table
    conn.execute(query("CREATE TABLE test_concurrent_mix (id INTEGER PRIMARY KEY, counter INTEGER DEFAULT 0)")).await?;
    
    // Insert initial data
    for i in 0..10 {
        conn.execute(query("INSERT INTO test_concurrent_mix (id, counter) VALUES (?, 0)")
            .bind(i)).await?;
    }
    ...
    for writer_id in 0..3 {
        ...
        let handle = tokio::spawn(async move {
            for _ in 0..10 {
                let id = writer_id % 10;
                conn_clone.execute(query("UPDATE test_concurrent_mix SET counter = counter + 1 WHERE id = ?")
                    .bind(id)).await.unwrap();
...
async fn test_arguments_not_consumed() -> anyhow::Result<()> {
    ...
    // Setup test table
    conn.execute(query("CREATE TABLE test_args (id INTEGER, value TEXT)")).await?;
...
async fn test_connection_thread_safety() -> anyhow::Result<()> {
    ...
    // Setup test table
    conn.execute(query("CREATE TABLE test_thread_safety (id INTEGER, thread_name TEXT)")).await?;
        ...
        let handle = tokio::task::spawn_blocking(move || {
            tokio::runtime::Handle::current().block_on(async move {
                ...
                conn_clone.execute(query("INSERT INTO test_thread_safety (id, thread_name) VALUES (?, ?)")
                    .bind(i as i32)
                    .bind(&thread_name)).await?;
                
                // Read it back
                let result: (i32, String) = query_as("SELECT id, thread_name FROM test_thread_safety WHERE id = ?")
                    .bind(i as i32)
                    .fetch_one(&*conn_clone)
                    .await?;
