...
use crate::sqlite::pool::PoolOptions;
use crate::sqlite::testing::{FixtureSnapshot, TestArgs, TestContext, TestSupport};
...
impl TestSupport for Sqlite {
    ...
    fn cleanup_test(db_name: &str) -> BoxFuture<'_, Result<(), Error>> {
        Box::pin(async move { Ok(crate::sqlite::fs::remove_file(db_name).await?) })
    }

    fn cleanup_test_dbs() -> BoxFuture<'static, Result<Option<usize>, Error>> {
        Box::pin(async move {
            crate::sqlite::fs::remove_dir_all(BASE_PATH).await?;
            Ok(None)
        })
    }
...
async fn test_context(args: &TestArgs) -> Result<TestContext<Sqlite>, Error> {
    ...
    if let Some(parent_path) = Path::parent(db_path.as_ref()) {
        crate::sqlite::fs::create_dir_all(parent_path)
            .await
            .expect("failed to create folders");
    }

    if Path::exists(db_path.as_ref()) {
        crate::sqlite::fs::remove_file(&db_path)
            .await
